...
for ns in test prod; do
  ...
  helm uninstall config-server -n "$ns" || true
  helm uninstall eureka-server -n "$ns" || true
  helm uninstall exchange-generator -n "$ns" || true
  helm uninstall exchange-service -n "$ns" || true
  helm uninstall frontend -n "$ns" || true
  helm uninstall gateway -n "$ns" || true
...
for ns in test prod; do
  kubectl delete secret account-service-account-db -n "$ns" --ignore-not-found
done
...
docker image rm ${DOCKER_REGISTRY}/config-server:1 || true
docker image rm ${DOCKER_REGISTRY}/eureka-server:1 || true
docker image rm ${DOCKER_REGISTRY}/exchange-generator:1 || true
docker image rm ${DOCKER_REGISTRY}/exchange-service:1 || true
docker image rm ${DOCKER_REGISTRY}/frontend:1 || true
docker image rm ${DOCKER_REGISTRY}/gateway:1 || true
docker image rm ${DOCKER_REGISTRY}/notification-service:1 || true
